...
from dinov2.eval.setup import setup_and_build_lora_model
...
class CLIPRModel(torch.nn.Module):
    def __init__(self, vision_type='resnet_v1', bert_type='emilyalsentzer/Bio_ClinicalBERT', vision_pretrained=True,
                 ...
                 norm_features=True,R=16,args=None):
        super().__init__()
        ...
        self.vision_model = VisionModel(vision_type=self.vision_type, pretrained=self.vision_pretrained,
                                        proj_dim=self.proj_dim, proj_bias=self.proj_bias,
                                        projection=self.projection,
                                        norm=self.norm_features,R=R,args=args)
        ...
        if from_checkpoint:
            # print("self.weights_path ========= {}".format(self.weights_path))
...
class VisionModel(torch.nn.Module):
    def __init__(self, vision_type='resnet',
                 pretrained=True, proj_dim=512, proj_bias=False,
                 projection=True,
                 norm=True,R=16,args=None):
        super().__init__()
        self.proj_dim = proj_dim

        # Assert vision encoders
        if vision_type not in ['lora', 'vit_large_patch32_224','DinoV2']:
            print("Vision model should be one of 'lora', 'vit_large_patch32_224','DinoV2'...")
            import sys
            sys.exit()

        # Set vision encoder architecture and pretrained weights
        if vision_type == "DinoV2":
            self.model, self.autocast_dtype = setup_and_build_lora_model(args)
...
class TextModel(torch.nn.Module):
    ...
    def tokenize(self, prompts_list):
        # print("prompts_list =========== {}".format(prompts_list))
