...
def save_checkpoint_epoch(state,pred_Auc,pred_ACC,mean_AUC_test, mean_ACC_test,
                          epoch,is_best,checkpoint_path,stage="val",
                          filename='./checkpoint/checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        print("Model Saving................")
        shutil.copyfile(filename, osp.join(checkpoint_path,'model_{}_Val_{:03d}_Test_{:.6f}_{:.6f}_External1_{:.6f}_External2_{:.6f}.pth.tar'.format(
            stage,(epoch + 1),pred_Auc,pred_ACC,mean_AUC_test, mean_ACC_test)))


# mean_ACC, Internal_ACC, External_1_ACC, External_2_ACC, epoch, True, checkpoint_dir, stage="Test",
#                 filename=os.path.join(checkpoint_dir,"checkpoint.pth.tar")
def save_checkpoint_notest(state,Val_Acc, Internal_ACC, External_1_ACC, External_2_ACC,
                          epoch,is_best,checkpoint_path,stage="val",
                          filename='./checkpoint/checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        print("Model Saving................")
        shutil.copyfile(filename, osp.join(checkpoint_path,'model_{}_Epoch_{:03d}_Val_{:.6f}_InternalTesting_{:.6f}_External1_{:.6f}_External2_{:.6f}.pth.tar'.format(
            stage,(epoch + 1),Val_Acc, Internal_ACC, External_1_ACC, External_2_ACC)))
